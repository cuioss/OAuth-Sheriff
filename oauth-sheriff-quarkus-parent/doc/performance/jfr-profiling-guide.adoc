...
# JFR output directory (owned by Quarkus non-root user)
RUN mkdir -p /tmp/jfr-output && chown 1001:0 /tmp/jfr-output && chmod 775 /tmp/jfr-output
...
**January 2026 JFR Findings** (48,027 execution samples, top-of-stack):
...
**TLS encryption is the largest CPU consumer**:

* HTTPS encryption/decryption accounts for 18.7% of top-of-stack samples
* This is transport security cost, present in any HTTPS application

**RSA/BigInteger operations are minimal**:

* Only 0.1% of CPU samples (46 of 48,027) involve BigInteger operations
* RSA signature verification takes 68µs (JMH verified)

**CUI Sheriff OAuth library overhead is minimal**:

* Only 0.3% of CPU samples in CUI library code
* Token validation overhead is dominated by I/O and TLS, not application logic
