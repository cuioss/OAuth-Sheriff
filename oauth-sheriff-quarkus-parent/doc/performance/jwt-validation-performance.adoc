...
The **1.4ms difference** between JWT (1.91ms) and Health (0.510ms) endpoints is explained by:

[cols="2,2,3", options="header"]
|===
| Layer | Additional Latency | What It Does

| CDI Producer Chain + JWT
| **1.087ms**
| `Instance.get()`, token parsing, RSA signature (68µs), claim validation

| JAX-RS Routing
| 0.274ms
| Method matching, parameter binding, response serialization
...
**Total additional overhead**: ~1.4ms (matches observed 1.91ms - 0.510ms = 1.4ms)

=== Key Insight

The primary latency contributor is **Layer 5: CDI Producer Chain + JWT Validation** at 1.087ms (57%). This includes:

* `Instance<BearerTokenResult>.get()` resolution (CDI overhead)
* Token extraction from Authorization header
* JWT parsing and RSA signature verification (68µs)
* Claim extraction and validation

NOTE: The JWT library itself (77µs) is only 5.5% of the 1.4ms overhead. The majority (93%) is CDI dependency injection and framework integration.
