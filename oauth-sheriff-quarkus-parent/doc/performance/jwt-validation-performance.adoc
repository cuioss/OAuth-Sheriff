...
The **1.4ms difference** between JWT (1.91ms) and Health (0.510ms) is explained by two measured gaps:

[cols="2,2,2,3", options="header"]
|===
| Gap | Measured Endpoints | Latency | What It Represents

| **Framework Overhead**
| Health → Mock JWT
| 0.313ms (22%)
| POST vs GET, JAX-RS routing, Jackson serialization

| **CDI + JWT Validation**
| Mock JWT → JWT
| **1.087ms (78%)**
| `Instance.get()`, producer chain, JWT library (77µs)
|===

**Total**: 0.313ms + 1.087ms = **1.4ms** ✓

=== Key Insight

The **Mock JWT endpoint** (`/mock-jwt/validate`) replicates the full HTTP processing path but skips actual JWT validation. This directly measures the decomposition:

* **Health endpoint**: 0.510ms — baseline infrastructure
* **Mock JWT endpoint**: 0.823ms — adds framework overhead (+0.313ms)
* **JWT endpoint**: 1.91ms — adds CDI producer + validation (+1.087ms)

Within the 1.087ms CDI + JWT gap:

* JWT library (parsing + RSA signature): **77µs** (7%)
* CDI producer overhead (`Instance.get()`, `@Timed`, concurrency): **~1.01ms** (93%)

NOTE: The JWT library itself is fast (77µs). The dominant cost is CDI's `Instance<BearerTokenResult>` proxy resolution and producer invocation per request.

See link:../../../benchmarking/doc/Analysis-01.2026-Latency-Decomposition.adoc[Latency Decomposition Analysis] for detailed methodology and ablation study.
