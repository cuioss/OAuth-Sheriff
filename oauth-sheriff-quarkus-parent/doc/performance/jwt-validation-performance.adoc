...
| **CDI Producer + JWT Validation**
| JWT − Mock JWT
| **1.087ms**
| **57%**
|===

**Total**: 0.510 + 0.313 + 1.087 = 1.91ms ✓

=== Reference: JMH Library Benchmark

The JMH microbenchmark measures the JWT library **in isolation** (no HTTP, no CDI):

* **Complete validation**: 77µs P50
* **RSA signature**: 68µs (88% of library time)
* **Parsing + claims**: 9µs

The 1.087ms integration gap is **14x slower** than JMH's 77µs. This difference includes CDI overhead (`Instance.get()`, `@Timed`) plus real-world effects (object allocation, concurrency, memory pressure) not present in JMH.

=== Why JWT is Slower than Health

The **1.4ms difference** between JWT (1.91ms) and Health (0.510ms) breaks down into two measured components:
...
**Total**: 0.313 + 1.087 = **1.4ms** ✓

=== Key Insight

The **Mock JWT endpoint** (`/mock-jwt/validate`) replicates the full HTTP path but skips `tokenValidator.createAccessToken()`. This directly measures the validation cost:

* **Health → Mock JWT** (0.313ms): Framework overhead (POST vs GET, JAX-RS, Jackson)
* **Mock JWT → JWT** (1.087ms): CDI producer chain + JWT validation

The 1.087ms gap includes:
...
* `tokenValidator.createAccessToken()` under 50 concurrent connections
* `AccessTokenContent` object construction

NOTE: JMH measures the JWT library at 77µs in isolation. The integration gap (1.087ms) is **14x slower** due to CDI overhead and real-world concurrency effects.
