...
The **Mock JWT endpoint** (`/mock-jwt/validate`) performs all HTTP processing but skips `tokenValidator.createAccessToken()`. This directly measures the token validation cost:

* **Mock JWT baseline**: 0.823ms — full framework path without validation
* **JWT endpoint**: 1.91ms — adds token validation
* **Token validation cost**: 1.087ms (JWT − Mock JWT)

This 1.087ms is the **application's token validation feature** — the cost of calling `tokenValidator.createAccessToken()` in the real integration context.

=== Reference: Library Performance

JMH microbenchmark measures the library **in isolation**: 77µs P50.

The application context (1.087ms) is **14x slower** than isolated library (77µs). This difference includes CDI producer invocation, `@Timed` instrumentation, real object allocation, and concurrency under 50 connections.
