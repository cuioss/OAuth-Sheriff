...
The extension provides optional default claim mappers for seamless integration with Keycloak's standard token structure. These mappers are configured globally via CDI beans and are disabled by default.

[cols="2,1,5"]
|===
|Property |Type |Description

|`sheriff.oauth.keycloak.default-roles-mapper.enabled`
|boolean (default: false)
|Enables the default roles mapper that transforms Keycloak's `realm_access.roles` claim to the standard `roles` claim. This allows applications to work with Keycloak tokens without requiring custom protocol mappers.

|`sheriff.oauth.keycloak.default-groups-mapper.enabled`
|boolean (default: false)
|Enables the default groups mapper that processes Keycloak's standard `groups` claim. This provides consistent group membership handling for Keycloak tokens.
|===
...
* **Global CDI-based Registration**: Claim mappers are discovered and registered via CDI using the `DiscoverableClaimMapper` SPI
...
* **Extensible**: Custom mappers can be added by implementing `DiscoverableClaimMapper` as CDI beans
