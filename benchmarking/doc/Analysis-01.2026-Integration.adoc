...
=== What Remains: CDI `Instance.get()` + Object Allocation

After ruling out the above factors, the 1.087ms token validation cost is attributed to:

1. **CDI `Instance<BearerTokenResult>` resolution** — each `Instance.get()` creates a `CreationalContextImpl` (250 JFR allocation samples)
2. **InjectionPoint introspection** — CDI extracts `@BearerToken` annotation parameters per request
3. **Object allocation** — HashMap, Set, and result objects created per request (3,915 HashMap-related JFR samples)
4. **Memory barriers** — Lock-free cache operations require memory ordering guarantees
...
Full JFR analysis: link:Analysis-01.2026-JFR-Profiling.adoc[JFR Profiling Analysis — Non-CPU Overhead]