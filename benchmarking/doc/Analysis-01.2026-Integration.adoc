...
The ~3.9x throughput difference (95K health vs 24K JWT at 100 conns) is fully explained by the ~4.5x per-request latency ratio. With a fixed number of connections, throughput is inversely proportional to per-request latency — if each JWT request takes 4.5x longer to process, throughput drops by approximately that factor. The slight difference between the throughput ratio (3.9x) and the latency ratio (4.5x) is accounted for by JWT's higher thread count (74 avg vs 35 avg at 50 conns) and correspondingly higher CPU usage (76.3% vs 70.9% avg), which partially compensates for the longer per-request time.
